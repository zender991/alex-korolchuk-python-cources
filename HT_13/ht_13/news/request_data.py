...
class Stories(object):
    ...
    def get_id_from_db(category):                                   # get id list of existing stories in DB
        Stories.cursor.execute(get_stories_id_query)
            ...
            if str(current_story['id']) not in id_list_from_db:               # compare received stories with exisitng in DB
                Stories.cursor.execute(insert_query,
                               (item_var[0], item_var[1], item_var[2], item_var[3], item_var[4],        # insert new story in DB
                                item_var[5], item_var[6], item_var[7], item_var[8], item_var[9], category_id))
            else:
                Stories.cursor.execute(update_query,
                               (item_var[0], item_var[1], item_var[2], item_var[3], item_var[4],        # update existing story in DB
                                item_var[5], item_var[6], item_var[7], item_var[8], item_var[9], category_id, str(current_story['id'])))
...
def execute():
    instance = Stories()
    instance.get_stories('askstories', insert_story_query, update_story_query, 1)
    instance.get_stories('newstories', insert_story_query, update_story_query, 2)
    instance.get_stories('jobstories', insert_story_query, update_story_query, 3)
    instance.get_stories('showstories', insert_story_query, update_story_query, 4)
